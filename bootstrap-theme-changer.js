...
$(function() {
  ...
  $('#bt-value-color').change(function() {
    ...
    for (var j = 0; j < btcolor.length; j++) {
      ...
      $('.btn').removeClass('text-'+btcolor[j]+' border-'+btcolor[j]); // Removing some unwanted {color} classes.
      $('kbd,.bg-'+btcolor[j]).removeClass('text-'+btcolor[j]); // Removing some unwanted {color} classes.
    }
